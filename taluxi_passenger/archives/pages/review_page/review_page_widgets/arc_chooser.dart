...
class ChooserState extends State<ArcChooser>
  ...
  double startAngle;
  var autoAnimationIsRuinning = false;
  ...
  void initState() {
    ...
    arcItems.add(ArcItem("MÉDIOCRE", [Color(0xFFF9D976), Color(0xfff39f86)],
        angleInRadiansByTwo + userAngle));
    arcItems.add(ArcItem("CORRECT", [Color(0xFF21e1fa), Color(0xff3bb8fd)],
        angleInRadiansByTwo + userAngle + (angleInRadians)));
    arcItems.add(ArcItem("BONNE", [Color(0xFF3ee98a), Color(0xFF41f7c7)],
        angleInRadiansByTwo + userAngle + (2 * angleInRadians)));
    arcItems.add(ArcItem("MAUVAISE", [Color(0xFFfe0944), Color(0xFFfeae96)],
    ...
    timer = Timer.periodic(Duration(seconds: 5), (_) async {
      autoAnimationIsRuinning = true;
      animationEnd *= -1;
      await animation.animateTo(0.5, duration: Duration(milliseconds: 600));
      await animation.animateBack(0, duration: Duration(milliseconds: 600));
      // animationEnd = -1;
  ...
  Widget build(BuildContext context) {
    ...
    return new GestureDetector(
      onPanStart: (DragStartDetails details) {
        if (autoAnimationIsRuinning) {
          autoAnimationIsRuinning = false;
          timer.cancel();
          animation.reset();
          animationEnd = 0;
        }
